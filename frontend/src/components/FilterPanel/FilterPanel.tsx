...
import { SECTORS, INDEX_MEMBERSHIP, FilterConfig, FilterOperator } from '@/types/stock';
...
export function FilterPanel() {
  ...
  const setRangeFilter = (field: string, range: [number, number]) => {
    ...
    if (existing) {
      ...
      addFilter({ id: `${field}-${Date.now()}`, field: field as FilterConfig['field'], operator: 'between', value: range, enabled: true });
    }
  };
  ...
  const setMultiSelectFilter = (field: string, selected: string[]) => {
    ...
    if (existing) {
      ...
      addFilter({ id: `${field}-${Date.now()}`, field: field as FilterConfig['field'], operator: 'in', value: selected, enabled: true });
    }
  };
  ...
  const setSingleSelectFilter = (field: string, value: string | null) => {
    ...
    if (existing) {
      ...
      addFilter({ id: `${field}-${Date.now()}`, field: field as FilterConfig['field'], operator: 'eq', value, enabled: true });
    }
  };
      ...
      <SimpleBar className="h-full" style={{ maxHeight: '100%' }}>
        <div className="p-2">
          ...
          <FilterGroup title="Classification">
            <MultiSelectFilter label="Sector" options={[...SECTORS]} selected={(getFilterValue('sector')?.value as string[]) || []} onChange={(v) => setMultiSelectFilter('sector', v)} />
            <MultiSelectFilter label="Market Cap Category" options={['Large Cap', 'Mid Cap', 'Small Cap', 'Micro Cap']} selected={(getFilterValue('marketCapCategory')?.value as string[]) || []} onChange={(v) => setMultiSelectFilter('marketCapCategory', v)} />
            <MultiSelectFilter label="Index Membership" options={[...INDEX_MEMBERSHIP]} selected={(getFilterValue('indexMembership')?.value as string[]) || []} onChange={(v) => setMultiSelectFilter('indexMembership', v)} />
          </FilterGroup>
          ...
          <FilterGroup title="Technical">
            ...
            <SingleSelectFilter label="MACD Signal" options={['Bullish', 'Bearish', 'Neutral']} selected={(getFilterValue('macdSignal')?.value as string) || null} onChange={(v) => setSingleSelectFilter('macdSignal', v)} />
            <SingleSelectFilter label="Price vs SMA 50" options={['Above', 'Below']} selected={(getFilterValue('priceVsSma50')?.value as string) || null} onChange={(v) => setSingleSelectFilter('priceVsSma50', v)} />
            <SingleSelectFilter label="Price vs SMA 200" options={['Above', 'Below']} selected={(getFilterValue('priceVsSma200')?.value as string) || null} onChange={(v) => setSingleSelectFilter('priceVsSma200', v)} />
            <SingleSelectFilter label="Bollinger Position" options={['Above Upper', 'Within Bands', 'Below Lower']} selected={(getFilterValue('bollingerPosition')?.value as string) || null} onChange={(v) => setSingleSelectFilter('bollingerPosition', v)} />
            <SingleSelectFilter label="Volume vs 20D Avg" options={['Below', 'Above', '2x Above', '3x Above']} selected={(getFilterValue('volumeVsAvg')?.value as string) || null} onChange={(v) => setSingleSelectFilter('volumeVsAvg', v)} />
          </FilterGroup>

          <FilterGroup title="Custom">
            <BooleanFilter label="Watchlist Only" value={(getFilterValue('watchlistOnly')?.value as boolean) || false} onChange={(v) => setBooleanFilter('watchlistOnly', v)} />
          </FilterGroup>
        </div>
      </SimpleBar>
    </div>
