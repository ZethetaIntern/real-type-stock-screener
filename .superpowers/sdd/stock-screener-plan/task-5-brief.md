...
## Step 1: Rewrite stockStore.ts

Replace `frontend/src/stores/stockStore.ts` with:
...
Expected: Build succeeds (or fails only due to pre-existing page.tsx issue).

## Step 3: Commit

```bash
git add src/stores/stockStore.ts
git commit -m "feat(store): expand Zustand store with FilterConfig[] and UI state"
```
